...
import NeonTigerSocietyABI from '../abis/NeonTigerSociety.json';
import Staking_NTSctyABI from '../abis/Staking_NTScty.json';

import { SocietyCoinContract, NeonTigerSociety, Staking_NTScty } from '../global/constants'
...
const NeonTigerCard = () => {
    ...
    const neonSocietyContract: any = useNoWalletContract(NeonTigerSocietyABI.abi, NeonTigerSociety);
    const stakingContract: any = useNoWalletContract(Staking_NTSctyABI.abi, Staking_NTScty);
    ...
    const copyAddress = () => {
        navigator.clipboard.writeText(NeonTigerSociety);
    ...
    const approve = async () => {
        try {
            await societyContract.approve(Staking_NTScty, ethers.constants.MaxUint256).then((tx: any) => {
    ...
    const transfer = async () => {
        try {
            ...
            await neonSocietyContract.transfer(claimAddress, ethers.utils.parseEther(claimAmount)).then((tx: any) => {
                tx.wait().then((response: any) => {
    ...
    const checkApproved = async () => {
        try {
            let allowance = await societyContract.allowance(account, Staking_NTScty);
    ...
    const checkBalance = async () => {
        try {
            if(account){
                const blnc = await neonSocietyContract.balanceOf(account);
        ...
        <div>
            <div className={`coin-total`}>
                <div className='coin-img'>
                    <img src='./img/neontiger.png' alt='imgs' className='coin-img-coin' />
                </div>
                <div className='coin-content'>
                    ...
                    <div className='coin-gift'>
                        <span>{'CHAMPION ANIMAL WELFARE & LOCAL PRODUCT SUPPLY CHAIN'}</span>
                        {/* <p style={{ display: 'flex', alignItems: 'center' }}>
                            {(
                                <>
                                    <span>
                    ...
                    <div className='coin-button-group' style={{width:"100%"}}>
                        
                        <button
                            className='coin-button-receive coin-button-receive1'
                            onClick={handleOpena}
...
export default memo(NeonTigerCard)
